        ...
        if packet.haslayer(TCP):
            ...
            if packet.haslayer(Raw):
                ...
                if any(keyword in payload.lower() for keyword in keywords):
                    print("\n" + "!" * 50)
                    print(f"[ALERT] Potential Sensitive Data Captured via {proto_name}!")
                    print(f"From: {ip_src} to {ip_dst}")
                    print(f"Packet Content Snippet: \n{payload.strip()[:300]}")
                    print("!" * 50 + "\n")

# main program execution
print("---- Professional Network Packet Sniffer ----")
print("[*] Initializing Network Interface Listener...")
print("[*] Sniffing in live traffic. Press Ctrl+C to stop.")
print("-" * 60)
